...
public class ActivationBuilder
{
    ...
    public T LocateWithNamedParameters<T>(params (string name, object value)[] parameters) => (T)Locate(typeof(T), null, parameters.Select(p => new NamedParameter(p.name, p.value)).ToArray());

    public T LocateWithTypedParams<T>(params object[] parameters) => (T)Locate(typeof(T), null, parameters.Select(p => new TypedParameter(p)).ToArray());

    public T Locate<T>(params IDIParameter[] parameters) => (T)Locate(typeof(T), null, parameters);

    public T Locate<T>(int position, object value) => (T)Locate(typeof(T), null, new PositionalParameter(position, value));

    public T LocateWithParams<T>(params IDIParameter[] parameters) => (T)Locate(typeof(T), null, parameters);

    public object Locate(Type type, Type? owner, params IDIParameter[] parameters)
    {
        ...
        if (_alias.TryGetValue(type, out var alias))
        {
            if (isEnumerable)
            {
                ...
                {
                    if (LocateInternal(a, isEnumerable, owner, parameters) is IList listOfObj)
            ...
            {
                foreach (var a in alias)
                    return LocateInternal(a, isEnumerable, owner, parameters);
        ...
        {
            return LocateInternal(type, isEnumerable, owner, parameters);
        }
    }

    private object LocateInternal(Type type, bool isEnumerable, Type? owner, params IDIParameter[] parameters)
    {
        ...
        {
            var retVal = isEnumerable ? instance : instance.First();

            return type == _loggerType && owner != null ? ((ILogger)retVal).ForContext(owner) : retVal;
        }
    ...
    private bool Locate(CachedConstructor c, Type type, LifestyleType lifestyleType, List<IDIParameter> parameterValues, out object val)
    {
        ...
        foreach (var p in c.Parameters)
        {
            ...
            parameters[i] = Locate(p.ParameterType, type);
            i++;
        }
