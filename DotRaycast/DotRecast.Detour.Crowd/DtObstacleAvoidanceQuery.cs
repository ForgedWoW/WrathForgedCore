...
public class DtObstacleAvoidanceQuery
{
    ...
    public void AddSegment(RcVec3f p, RcVec3f q)
    {
        ...
        var seg = m_segments[m_nsegments++];
        seg.p = p;
        seg.q = q;
    }
    ...
    private void Prepare(RcVec3f pos, RcVec3f dvel)
    {
        ...
        for (var i = 0; i < m_ncircles; ++i)
        {
            ...
            var orig = new RcVec3f();
            _ = new RcVec3f();
            cir.dp = RcVec3f.Subtract(pb, pa);
            cir.dp = RcVec3f.Normalize(cir.dp);
            var dv = RcVec3f.Subtract(cir.dvel, dvel);

            var a = DtUtils.TriArea2D(orig, cir.dp, dv);
            if (a < 0.01f)
            {
                cir.np.X = -cir.dp.Z;
                cir.np.Z = cir.dp.X;
 ...
 */
        ...
        float minPenalty, DtObstacleAvoidanceDebugData debug)
    {
        ...
        var vpen = m_params.weightDesVel * (RcVecUtils.Dist2D(vcand, dvel) * m_invVmax);
        var vcpen = m_params.weightCurVel * (RcVecUtils.Dist2D(vcand, vel) * m_invVmax);

        // find the threshold hit time to bail out based on the early out penalty
        // (see how the penalty is calculated below to understand)
        var minPen = minPenalty - vpen - vcpen;
        var tThresold = ((m_params.weightToi / minPen) - 0.1f) * m_params.horizTime;
        ...
        for (var i = 0; i < m_nsegments; ++i)
        {
            ...
            if (seg.touch)
            {
                ...
                var sdir = RcVec3f.Subtract(seg.q, seg.p);
                var snorm = new RcVec3f
                {
                    X = -sdir.Z,
                    Z = sdir.X
                };
        ...
        var spen = m_params.weightSide * side;
        var tpen = m_params.weightToi * (1.0f / (0.1f + (tmin * m_invHorizTime)));
        ...
        DtObstacleAvoidanceParams option, DtObstacleAvoidanceDebugData debug)
    {
        ...
        var cvx = dvel.X * m_params.velBias;
        var cvz = dvel.Z * m_params.velBias;
        var cs = vmax * 2 * (1 - m_params.velBias) / (m_params.gridSize - 1);
        var half = (m_params.gridSize - 1) * cs * 0.5f;

        var minPenalty = float.MaxValue;
        var ns = 0;

        for (var y = 0; y < m_params.gridSize; ++y)
        {
            for (var x = 0; x < m_params.gridSize; ++x)
            {
                var vcand = new RcVec3f(cvx + (x * cs) - half, 0f, cvz + (y * cs) - half);
                if (RcMath.Sqr(vcand.X) + RcMath.Sqr(vcand.Z) > RcMath.Sqr(vmax + (cs / 2)))
                    continue;

                var penalty = ProcessSample(vcand, cs, pos, rad, vel, dvel, minPenalty, debug);
                ns++;
    ...
    private void DtNormalize2D(float[] v)
    {
        var d = MathF.Sqrt((v[0] * v[0]) + (v[2] * v[2]));
        if (d == 0)
            return;
        d = 1.0f / d;
        v[0] *= d;
    ...
    // vector normalization that ignores the y-component.
    private RcVec3f DtRotate2D(float[] v, float ang)
    {
        var dest = new RcVec3f();
        var c = MathF.Cos(ang);
        var s = MathF.Sin(ang);
        dest.X = (v[0] * c) - (v[2] * s);
        dest.Z = (v[0] * s) + (v[2] * c);
    ...
    private static readonly float DT_PI = 3.14159265f;
        ...
        DtObstacleAvoidanceDebugData debug)
    {
        ...
        var pat = new float[((DT_MAX_PATTERN_DIVS * DT_MAX_PATTERN_RINGS) + 1) * 2];
        var npat = 0;

        var ndivs = m_params.adaptiveDivs;
        var nrings = m_params.adaptiveRings;
        var depth = m_params.adaptiveDepth;

        var nd = Math.Clamp(ndivs, 1, DT_MAX_PATTERN_DIVS);
        var nr = Math.Clamp(nrings, 1, DT_MAX_PATTERN_RINGS);
        var da = 1.0f / nd * DT_PI * 2;
        var ca = MathF.Cos(da);
        var sa = MathF.Sin(da);

        // desired direction
        var ddir = new float[6];
        ddir[0] = dvel.X;
        ddir[1] = dvel.Y;
        ddir[2] = dvel.Z;
        DtNormalize2D(ddir);
        ...
        // Start sampling.
        var cr = vmax * (1.0f - m_params.velBias);
        var res = new RcVec3f(dvel.X * m_params.velBias, 0, dvel.Z * m_params.velBias);
        var ns = 0;
        for (var k = 0; k < depth; ++k)
        {
            var minPenalty = float.MaxValue;
            _ = new RcVec3f();
            var bvel = RcVec3f.Zero;

            for (var i = 0; i < npat; ++i)
            {
                var vcand = new RcVec3f(res.X + (pat[(i * 2) + 0] * cr), 0f, res.Z + (pat[(i * 2) + 1] * cr));
                if (RcMath.Sqr(vcand.X) + RcMath.Sqr(vcand.Z) > RcMath.Sqr(vmax + 0.001f))
                    continue;

                var penalty = ProcessSample(vcand, cr / 10, pos, rad, vel, dvel, minPenalty, debug);
                ns++;
                if (penalty < minPenalty)
                {
                    minPenalty = penalty;
