...
public class DtLocalBoundary
{
    ...
    private RcVec3f m_center = new();
    private readonly List<DtSegment> m_segs = [];
    private List<long> m_polys = [];
    private List<long> m_parents = [];

    public DtLocalBoundary() => m_center.X = m_center.Y = m_center.Z = float.MaxValue;
    ...
    public RcVec3f GetCenter() => m_center;

    public RcVec3f[] GetSegment(int j) => m_segs[j].s;

    public int GetSegmentCount() => m_segs.Count;
}