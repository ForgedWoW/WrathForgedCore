...
{
    private RcVec3f m_pos = new();
    private RcVec3f m_target = new();
    private List<long> m_path;

    /**
 * Allocates the corridor's path buffer.
 */
    public DtPathCorridor() => m_path = [];
 ...
 */
    public int FindCorners(ref List<DtStraightPath> corners, int maxCorners, DtNavMeshQuery navquery, IDtQueryFilter filter)
    {
        ...
        if (result.Succeeded())
        {
            ...
            var end = corners.Count;
            // Prune points after an off-mesh connection.
            for (var i = start; i < corners.Count; i++)
            {
                var spi = corners[i];
                if ((spi.flags & DtStraightPathFlags.DT_STRAIGHTPATH_OFFMESH_CONNECTION) != 0)
                {
 ...
 */
    public void OptimizePathVisibility(RcVec3f next, float pathOptimizationRange, DtNavMeshQuery navquery, IDtQueryFilter filter)
    {
        ...
        var dist = RcVecUtils.Dist2D(m_pos, next);
        ...
        var goal = RcVecUtils.Mad(m_pos, delta, pathOptimizationRange / dist);
 ...
 */
    public bool OptimizePathTopology(DtNavMeshQuery navquery, IDtQueryFilter filter, int maxIterations)
    {
        ...
        _ = navquery.InitSlicedFindPath(m_path[0], m_path[^1], m_pos, m_target, filter, 0);
        _ = navquery.UpdateSlicedFindPath(maxIterations, out _);
 ...
 */
    ...
    public bool TrimInvalidPath(long safeRef, float[] safePos, DtNavMeshQuery navquery, IDtQueryFilter filter)
    {
        ...
        _ = navquery.ClosestPointOnPolyBoundary(m_path[^1], m_target, out m_target);
        return true;
    }
 ...
 */
    public bool IsValid(int maxLookAhead, DtNavMeshQuery navquery, IDtQueryFilter filter)
    {
        ...
        var n = Math.Min(m_path.Count, maxLookAhead);
        for (var i = 0; i < n; ++i)
        {
 ...
 */
    public RcVec3f GetTarget() => m_target;
 ...
 */
    public long GetFirstPoly() => 0 == m_path.Count ? 0 : m_path[0];
 ...
 */
    public long GetLastPoly() => 0 == m_path.Count ? 0 : m_path[^1];

    /**
 * The corridor's path.
 */
    public List<long> GetPath() => m_path;
 ...
 */
    public int GetPathCount() => m_path.Count;
}