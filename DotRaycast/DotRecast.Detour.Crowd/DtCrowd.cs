...
public class DtCrowd
{
    ...
    private readonly RcVec3f _ext = new();
    ...
    public DtCrowd(DtCrowdConfig config, DtNavMesh nav, Func<int, IDtQueryFilter> queryFilterFactory)
    {
        ...
        for (var i = 0; i < DT_CROWD_MAX_QUERY_FILTER_TYPE; i++)
        {
            _filters[i] = queryFilterFactory.Invoke(i);
        }

        // Init obstacle query option.
        for (var i = 0; i < DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS; ++i)
        {
    ...
    public DtNavMesh GetNavMesh() => _navMesh;

    public DtNavMeshQuery GetNavMeshQuery() => _navQuery;
    ...
    {
        if (idx is >= 0 and < DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS)
        {
    ...
    public DtObstacleAvoidanceParams GetObstacleAvoidanceParams(int idx) => idx is >= 0 and < DT_CROWD_MAX_OBSTAVOIDANCE_PARAMS ? _obstacleQueryParams[idx] : null;

    /// Updates the specified agent's configuration.
    /// @param[in] idx The agent index. [Limits: 0 <= value < #GetAgentCount()]
    /// @param[in] params The new agent configuration.
    public void UpdateAgentParameters(DtCrowdAgent agent, DtCrowdAgentParams option) => agent.option = option;
 ...
 */
    ...
    {
        var ag = new DtCrowdAgent(_agentId.GetAndIncrement());
        _agents.Add(ag);
        ...
        ag.state = refs != 0 ? DtCrowdAgentState.DT_CROWDAGENT_STATE_WALKING : DtCrowdAgentState.DT_CROWDAGENT_STATE_INVALID;
 ...
 *            Agent to be removed
 */
    public void RemoveAgent(DtCrowdAgent agent) => _agents.Remove(agent);
 ...
 */
    public IList<DtCrowdAgent> GetActiveAgents() => _agents;

    public RcVec3f GetQueryExtents() => _ext;

    public IDtQueryFilter GetFilter(int i) => i is >= 0 and < DT_CROWD_MAX_QUERY_FILTER_TYPE ? _filters[i] : null;

    public DtProximityGrid GetGrid() => _grid;

    public DtPathQueue GetPathQueue() => _pathQ;

    public DtCrowdTelemetry Telemetry() => _telemetry;

    public DtCrowdConfig Config() => _config;
    ...
    private void CheckPathValidity(IList<DtCrowdAgent> agents, float dt)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            var agentPos = ag.npos;
            if (!_navQuery.IsValidPolyRef(agentRef, _filters[ag.option.queryFilterType]))
            {
                // Current location is not valid, try to reposition.
                // TODO: this can snap agents, how to handle that?
                _ = _navQuery.FindNearestPoly(ag.npos, _ext, _filters[ag.option.queryFilterType], out agentRef, out var nearestPt, out _);
                agentPos = nearestPt;
            ...
            if (ag.targetState is DtMoveRequestState.DT_CROWDAGENT_TARGET_NONE
                or DtMoveRequestState.DT_CROWDAGENT_TARGET_VELOCITY)
            {
                continue;
            }

            // Try to recover move request position.
            if (ag.targetState is not DtMoveRequestState.DT_CROWDAGENT_TARGET_NONE
                and not DtMoveRequestState.DT_CROWDAGENT_TARGET_FAILED)
            {
                if (!_navQuery.IsValidPolyRef(ag.targetRef, _filters[ag.option.queryFilterType]))
                {
                    // Current target is not valid, try to reposition.
                    _ = _navQuery.FindNearestPoly(ag.targetPos, _ext, _filters[ag.option.queryFilterType], out ag.targetRef, out var nearestPt, out _);
            ...
            if (replan)
            {
                ...
                {
                    _ = RequestMoveTargetReplan(ag, ag.targetRef, ag.targetPos);
                }
            }
        }
    }
    ...
    private void UpdateMoveRequest(IList<DtCrowdAgent> agents, float dt)
    {
        ...
        var queue = new RcSortedQueue<DtCrowdAgent>((a1, a2) => a2.targetReplanTime.CompareTo(a1.targetReplanTime));
        ...
        foreach (var ag in agents)
        {
            ...
            if (ag.targetState == DtMoveRequestState.DT_CROWDAGENT_TARGET_REQUESTING)
            {
                ...
                _ = _navQuery.InitSlicedFindPath(path[0], ag.targetRef, ag.npos, ag.targetPos,
                    _filters[ag.option.queryFilterType], 0);
                _ = _navQuery.UpdateSlicedFindPath(_config.maxTargetFindPathIterations, out _);
                ...
                if (status.Succeeded() && reqPath.Count > 0)
                {
                    ...
                    if (reqPath[^1] != ag.targetRef)
                    {
                        ...
                        // last polygon.
                        var cr = _navQuery.ClosestPointOnPoly(reqPath[^1], ag.targetPos, out reqPos, out _);
                        if (cr.Failed())
                        {
                            reqPath = [];
                        }
        ...
        foreach (var ag in agents)
        {
            ...
            if (ag.targetState == DtMoveRequestState.DT_CROWDAGENT_TARGET_WAITING_FOR_PATH)
            {
                ...
                // Poll path queue.
                var status = ag.targetPathQueryResult.status;
                if (status.Failed())
                {
                    // Path find failed, retry if the target location is still
                    // valid.
                    ag.targetPathQueryResult = null;
                    ag.targetState = ag.targetRef != 0 ? DtMoveRequestState.DT_CROWDAGENT_TARGET_REQUESTING : DtMoveRequestState.DT_CROWDAGENT_TARGET_FAILED;
                ...
                else if (status.Succeeded())
                {
                    ...
                    if (valid)
                    {
                        ...
                        if (res[^1] != ag.targetRef)
                        {
                            ...
                            // the last polygon.
                            var cr = _navQuery.ClosestPointOnPoly(res[^1], targetPos, out var nearest, out _);
                            if (cr.Succeeded())
                            {
                                targetPos = nearest;
                            }
    ...
    private void UpdateTopologyOptimization(IList<DtCrowdAgent> agents, float dt)
    {
        ...
        var queue = new RcSortedQueue<DtCrowdAgent>((a1, a2) => a2.topologyOptTime.CompareTo(a1.topologyOptTime));
        ...
        {
            var ag = queue.Dequeue();
            _ = ag.corridor.OptimizePathTopology(_navQuery, _filters[ag.option.queryFilterType], _config.maxTopologyOptimizationIterations);
            ag.topologyOptTime = 0;
        }
    }
    ...
    private void BuildNeighbours(IList<DtCrowdAgent> agents)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            var updateThr = ag.option.collisionQueryRange * 0.25f;
            ...
            _ = GetNeighbours(ag.npos, ag.option.height, ag.option.collisionQueryRange, ag, ref ag.neis, _grid);
        }
    }
    ...
    private int GetNeighbours(RcVec3f pos, float height, float range, DtCrowdAgent skip, ref List<DtCrowdNeighbour> result, DtProximityGrid grid)
    {
        ...
        var nids = grid.QueryItems(pos.X - range, pos.Z - range, pos.X + range, pos.Z + range, ref proxAgents);
        foreach (var ag in proxAgents)
        {
            if (ag == skip)
            {
                continue;
            ...
            // Check for overlap.
            var diff = RcVec3f.Subtract(pos, ag.npos);
            ...
            diff.Y = 0;
            var distSqr = diff.LengthSquared();
    ...
    private void FindCorners(IList<DtCrowdAgent> agents, DtCrowdAgentDebugInfo debug)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            _ = ag.corridor.FindCorners(ref ag.corners, DT_CROWDAGENT_MAX_CORNERS, _navQuery, _filters[ag.option.queryFilterType]);

            // Check to see if the corner after the next corner is directly visible,
            // and short cut to there.
            if ((ag.option.updateFlags & DtCrowdAgentUpdateFlags.DT_CROWD_OPTIMIZE_VIS) != 0 && ag.corners.Count > 0)
            {
                var target = ag.corners[Math.Min(1, ag.corners.Count - 1)].pos;
    ...
    private void TriggerOffMeshConnections(IList<DtCrowdAgent> agents)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            // Check
            var triggerRadius = ag.option.radius * 2.25f;
            if (ag.OverOffmeshConnection(triggerRadius))
            {
                // Prepare to off-mesh connection.
                var anim = ag.animation;
    ...
    private void CalculateSteering(IList<DtCrowdAgent> agents)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            else
            {
                ...
                dvel = (ag.option.updateFlags & DtCrowdAgentUpdateFlags.DT_CROWD_ANTICIPATE_TURNS) != 0
                    ? ag.CalcSmoothSteerDirection()
                    : ag.CalcStraightSteerDirection();

                // Calculate speed scale, which tells the agent to slowdown at the end of the path.
                var slowDownRadius = ag.option.radius * 2; // TODO: make less hacky.
                var speedScale = ag.GetDistanceToGoal(slowDownRadius) / slowDownRadius;
            ...
            {
                var separationDist = ag.option.collisionQueryRange;
                var invSeparationDist = 1.0f / separationDist;
                var separationWeight = ag.option.separationWeight;

                float w = 0;
                var disp = new RcVec3f();

                for (var j = 0; j < ag.neis.Count; ++j)
                {
                    var nei = ag.neis[j].agent;

                    var diff = RcVec3f.Subtract(ag.npos, nei.npos);
                    diff.Y = 0;
                ...
                if (w > 0.0001f)
                {
                    ...
                    var speedSqr = dvel.LengthSquared();
                    var desiredSqr = RcMath.Sqr(ag.desiredSpeed);
    ...
    private void PlanVelocity(DtCrowdAgentDebugInfo debug, IList<DtCrowdAgent> agents)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            if ((ag.option.updateFlags & DtCrowdAgentUpdateFlags.DT_CROWD_OBSTACLE_AVOIDANCE) != 0)
            {
                ...
                for (var j = 0; j < ag.boundary.GetSegmentCount(); ++j)
                {
                ...
                var option = _obstacleQueryParams[ag.option.obstacleAvoidanceType];
        ...
        for (var iter = 0; iter < 4; ++iter)
        {
            foreach (var ag in agents)
            {
                ...
                for (var j = 0; j < ag.neis.Count; ++j)
                {
                    ...
                    var pen = ag.option.radius + nei.option.radius - dist;
                    if (dist < 0.0001f)
                    {
                        // Agents on top of each other, try to choose diverging separation directions.
                        diff = idx0 > idx1 ? new RcVec3f(-ag.dvel.Z, 0, ag.dvel.X) : new RcVec3f(ag.dvel.Z, 0, -ag.dvel.X);
    ...
    private void MoveAgents(IList<DtCrowdAgent> agents)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            // Move along navmesh.
            _ = ag.corridor.MovePosition(ag.npos, _navQuery, _filters[ag.option.queryFilterType]);
    ...
    private void UpdateOffMeshConnections(IList<DtCrowdAgent> agents, float dt)
    {
        ...
        foreach (var ag in agents)
        {
            ...
            // Update position
            var ta = anim.tmax * 0.15f;
            var tb = anim.tmax;
            if (anim.t < ta)
            {
                var u = Tween(anim.t, 0.0f, ta);
    ...
    private float Tween(float t, float t0, float t1) => Math.Clamp((t - t0) / (t1 - t0), 0.0f, 1.0f);
}