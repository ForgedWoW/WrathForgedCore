...
public class DtObstacleAvoidanceDebugData
{
    ...
    public void Reset() => m_nsamples = 0;

    private void NormalizeArray(float[] arr, int n)
    {
        // Normalize penaly range.
        var minPen = float.MaxValue;
        var maxPen = -float.MaxValue;
        for (var i = 0; i < n; ++i)
        {
        ...
        var s = penRange > 0.001f ? (1.0f / penRange) : 1;
        for (var i = 0; i < n; ++i)
    ...
    public int GetSampleCount() => m_nsamples;
    ...
    public float GetSampleSize(int i) => m_ssize[i];

    public float GetSamplePenalty(int i) => m_pen[i];

    public float GetSampleDesiredVelocityPenalty(int i) => m_vpen[i];

    public float GetSampleCurrentVelocityPenalty(int i) => m_vcpen[i];

    public float GetSamplePreferredSidePenalty(int i) => m_spen[i];

    public float GetSampleCollisionTimePenalty(int i) => m_tpen[i];
}