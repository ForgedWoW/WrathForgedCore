...
public class DtCrowdTelemetry
{
    ...
    private readonly Dictionary<DtCrowdTimerLabel, long> _executionTimings = [];
    private readonly Dictionary<DtCrowdTimerLabel, List<long>> _executionTimingSamples = [];

    public float MaxTimeToEnqueueRequest() => _maxTimeToEnqueueRequest;

    public float MaxTimeToFindPath() => _maxTimeToFindPath;
    ...
    public void RecordMaxTimeToEnqueueRequest(float time) => _maxTimeToEnqueueRequest = Math.Max(_maxTimeToEnqueueRequest, time);

    public void RecordMaxTimeToFindPath(float time) => _maxTimeToFindPath = Math.Max(_maxTimeToFindPath, time);
    ...
    private void Start(DtCrowdTimerLabel name) => _executionTimings.Add(name, RcFrequency.Ticks);
