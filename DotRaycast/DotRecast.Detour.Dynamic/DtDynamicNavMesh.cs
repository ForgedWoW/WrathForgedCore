...
public class DtDynamicNavMesh
{
    ...
    private readonly Dictionary<long, DtDynamicTile> _tiles = [];
    private readonly RcTelemetry telemetry;
    private readonly DtNavMeshParams navMeshParams;
    private readonly BlockingCollection<IDtDaynmicTileJob> updateQueue = [];
 ...
 */
    public DtVoxelQuery VoxelQuery() => new(navMeshParams.orig, navMeshParams.tileWidth, navMeshParams.tileHeight, LookupHeightfield);

    private RcHeightfield LookupHeightfield(int x, int z) => GetTileAt(x, z)?.checkpoint.heightfield;

    public long AddCollider(IDtCollider collider)
    {
        var cid = currentColliderId.IncrementAndGet();
        updateQueue.Add(new DtDynamicTileColliderAdditionJob(cid, collider, GetTiles(collider.Bounds())));
        return cid;
    }

    public void RemoveCollider(long colliderId) => updateQueue.Add(new DtDynamicTileColliderRemovalJob(colliderId, GetTilesByCollider(colliderId)));
 ...
 */
    ...
    {
        _ = ProcessQueue();
        _ = Rebuild(_tiles.Values);
    }

    /**
 * Perform incremental update of the nav mesh
 */
    public bool Update() => Rebuild(ProcessQueue());
 ...
 */
    public Task<bool> Update(TaskFactory executor) => Rebuild(ProcessQueue(), executor);
    ...
    private ICollection<DtDynamicTile> GetTiles(float[] bounds)
    {
        ...
        var minx = (int)MathF.Floor((bounds[0] - navMeshParams.orig.X) / navMeshParams.tileWidth);
        var minz = (int)MathF.Floor((bounds[2] - navMeshParams.orig.Z) / navMeshParams.tileHeight);
        var maxx = (int)MathF.Floor((bounds[3] - navMeshParams.orig.X) / navMeshParams.tileWidth);
        var maxz = (int)MathF.Floor((bounds[5] - navMeshParams.orig.Z) / navMeshParams.tileHeight);
        List<DtDynamicTile> tiles = [];
        for (var z = minz; z <= maxz; ++z)
        {
            for (var x = minx; x <= maxx; ++x)
            {
                var tile = GetTileAt(x, z);
                if (tile != null)
                {
                    tiles.Add(tile);
                }
    ...
    private List<DtDynamicTile> GetTilesByCollider(long cid) => _tiles.Values.Where(t => t.ContainsCollider(cid)).ToList();

    private void Rebuild(DtDynamicTile tile)
    {
        _ = new DtNavMeshCreateParams
        {
    ...
    private bool UpdateNavMesh()
    {
        if (dirty)
        {
            var navMesh = new DtNavMesh(navMeshParams, MAX_VERTS_PER_POLY);
    ...
    private long LookupKey(long x, long z) => (z << 32) | x;

    public List<DtVoxelTile> VoxelTiles() => _tiles.Values.Select(t => t.voxelTile).ToList();

    public List<RcBuilderResult> RecastResults() => _tiles.Values.Select(t => t.recastResult).ToList();
}