...
public class DtDynamicTile(DtVoxelTile voxelTile)
{
    public readonly DtVoxelTile voxelTile = voxelTile;
    ...
    public bool Build(RcBuilder builder, DtDynamicNavMeshConfig config, RcTelemetry telemetry)
    {
        if (dirty)
        {
            var heightfield = BuildHeightfield(config, telemetry);
            var r = BuildRecast(builder, config, voxelTile, heightfield, telemetry);
            var option = NavMeshCreateParams(voxelTile.tileX, voxelTile.tileZ, voxelTile.cellSize,
    ...
    {
        var rasterizedColliders = checkpoint != null
            ? checkpoint.colliders as ICollection<long>
            : RcImmutableArray<long>.Empty;

        var heightfield = checkpoint != null
        ...
        RcHeightfield heightfield, RcTelemetry telemetry)
    {
        ...
        var r = builder.Build(vt.tileX, vt.tileZ, null, rcConfig, heightfield, telemetry);
    ...
    public bool ContainsCollider(long cid) => colliders.ContainsKey(cid);
