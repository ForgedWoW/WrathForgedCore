...
public class DtVoxelFileReader(IRcCompressor compressor)
{
    private readonly IRcCompressor _compressor = compressor;

    public DtVoxelFile Read(BinaryReader stream)
    {
        var buf = IOUtils.ToByteBuffer(stream);
        var file = new DtVoxelFile();
        ...
        var isExportedFromAstar = (file.version & DtVoxelFile.VERSION_EXPORTER_MASK) == 0;
        var compression = (file.version & DtVoxelFile.VERSION_COMPRESSION_MASK) == DtVoxelFile.VERSION_COMPRESSION_LZ4;
        ...
        var tileCount = buf.GetInt();
        for (var tile = 0; tile < tileCount; tile++)
        {
            var tileX = buf.GetInt();
            var tileZ = buf.GetInt();
            var width = buf.GetInt();
            var depth = buf.GetInt();
            var borderSize = buf.GetInt();
            var boundsMin = new RcVec3f
            {
                X = buf.GetFloat(),
                Y = buf.GetFloat(),
                Z = buf.GetFloat()
            };
            var boundsMax = new RcVec3f
            {
                X = buf.GetFloat(),
                Y = buf.GetFloat(),
                Z = buf.GetFloat()
            ...
            var cellSize = buf.GetFloat();
            var cellHeight = buf.GetFloat();
            var voxelSize = buf.GetInt();
            var position = buf.Position();
            var bytes = buf.ReadBytes(voxelSize).ToArray();
            if (compression)
            {
                bytes = _compressor.Decompress(bytes);
            }

            var data = new RcByteBuffer(bytes);
