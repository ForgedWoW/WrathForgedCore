...
public class DtVoxelFile
{
    ...
    public RcVec3f rotation = new();
    public float[] bounds = new float[6];
    public readonly List<DtVoxelTile> tiles = [];

    public void AddTile(DtVoxelTile tile) => tiles.Add(tile);
    ...
    public static DtVoxelFile From(DtDynamicNavMesh mesh)
    {
        ...
        foreach (var vt in mesh.VoxelTiles())
        {
            var heightfield = vt.Heightfield();
