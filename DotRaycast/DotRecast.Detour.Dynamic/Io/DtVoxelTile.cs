...
public class DtVoxelTile
{
    ...
    public RcHeightfield Heightfield() => DtVoxelFile.PREFERRED_BYTE_ORDER == RcByteOrder.BIG_ENDIAN ? HeightfieldBE() : HeightfieldLE();

    private RcHeightfield HeightfieldBE()
    {
        var hf = new RcHeightfield(width, depth, boundsMin, boundsMax, cellSize, cellHeight, borderSize);
        var position = 0;
        for (int z = 0, pz = 0; z < depth; z++, pz += width)
        {
            ...
            {
                RcSpan prev = null;
                var spanCount = RcByteUtils.GetShortBE(spanData, position);
                position += 2;
    ...
    private RcHeightfield HeightfieldLE()
    {
        ...
        for (int z = 0, pz = 0; z < depth; z++, pz += width)
        {
            ...
            {
                RcSpan prev = null;
                var spanCount = RcByteUtils.GetShortLE(spanData, position);
                position += 2;
                for (var s = 0; s < spanCount; s++)
                {
                    var span = new RcSpan
    ...
    {
        var counts = new int[heightfield.width * heightfield.height];
        var totalCount = 0;
        ...
        var data = new byte[(totalCount * SERIALIZED_SPAN_BYTES) + (counts.Length * SERIALIZED_SPAN_COUNT_BYTES)];
        var position = 0;
        for (int z = 0, pz = 0; z < heightfield.height; z++, pz += heightfield.width)
        {
            for (var x = 0; x < heightfield.width; x++)
    ...
    private byte[] ToByteArray(RcByteBuffer buf, int width, int height, RcByteOrder order)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < l; i++)
            {
                ...
                _ = RcByteUtils.PutShort(count, data, position, order);
                position += 2;
                for (var j = 0; j < count; j++)
                {
                    _ = RcByteUtils.PutInt(buf.GetInt(), data, position, order);
                    position += 4;
                    _ = RcByteUtils.PutInt(buf.GetInt(), data, position, order);
                    position += 4;
                    _ = RcByteUtils.PutInt(buf.GetInt(), data, position, order);
