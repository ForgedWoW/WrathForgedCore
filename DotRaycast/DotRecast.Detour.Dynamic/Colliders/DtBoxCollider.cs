...
public class DtBoxCollider(RcVec3f center, RcVec3f[] halfEdges, int area, float flagMergeThreshold) : DtCollider(area, flagMergeThreshold, Bounds(center, halfEdges))
{
    private readonly RcVec3f center = center;
    private readonly RcVec3f[] halfEdges = halfEdges;
    ...
    private static float[] Bounds(RcVec3f center, RcVec3f[] halfEdges)
    {
        ...
        for (var i = 0; i < 8; ++i)
        {
            ...
            var s1 = (i & 2) != 0 ? 1f : -1f;
            var s2 = (i & 4) != 0 ? 1f : -1f;
            var vx = center.X + (s0 * halfEdges[0].X) + (s1 * halfEdges[1].X) + (s2 * halfEdges[2].X);
            var vy = center.Y + (s0 * halfEdges[0].Y) + (s1 * halfEdges[1].Y) + (s2 * halfEdges[2].Y);
            var vz = center.Z + (s0 * halfEdges[0].Z) + (s1 * halfEdges[1].Z) + (s2 * halfEdges[2].Z);
