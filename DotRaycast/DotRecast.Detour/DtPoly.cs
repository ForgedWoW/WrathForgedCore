...
public class DtPoly(int index, int maxVertsPerPoly)
{
    public readonly int index = index;

    /** The indices of the polygon's vertices. The actual vertices are located in MeshTile::verts. */
    public readonly int[] verts = new int[maxVertsPerPoly];

    /** Packed data representing neighbor polygons references and flags for each edge. */
    public readonly int[] neis = new int[maxVertsPerPoly];
    ...
    public void SetPolyType(int t) => areaAndtype = (areaAndtype & 0x3f) | (t << 6);

    /** Gets the user defined area id. */
    public int GetArea() => areaAndtype & 0x3f;

    /** Gets the polygon type. (See: #dtPolyTypes) */
    public int GetPolyType() => areaAndtype >> 6;
}