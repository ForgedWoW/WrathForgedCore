...
/// < Sliced query state.
public class DtNavMeshQuery(DtNavMesh nav)
{
    /// < Add a vertex at every polygon edge crossing.
    protected readonly DtNavMesh m_nav = nav;

    protected readonly DtNodePool m_nodePool = new DtNodePool();
    protected readonly DtNodeQueue m_openList = new DtNodeQueue();
    ...
    public DtStatus FindRandomPoint(IDtQueryFilter filter, IRcRand frand, out long randomRef, out RcVec3f randomPt)
    {
        ...
        DtPoly poly = null;
        long polyRef = 0;
        var @base = m_nav.GetPolyRefBase(tile);
        ...
        _ = ClosestPointOnPoly(polyRef, pt, out var closest, out _);
 ...
 */
    public DtStatus FindRandomPointAroundCircle(long startRef, RcVec3f centerPos, float maxRadius,
        IDtQueryFilter filter, IRcRand frand, out long randomRef, out RcVec3f randomPt) => FindRandomPointAroundCircle(startRef, centerPos, maxRadius, filter, frand, DtNoOpDtPolygonByCircleConstraint.Shared, out randomRef, out randomPt);
 ...
 */
    public DtStatus FindRandomPointWithinCircle(long startRef, RcVec3f centerPos, float maxRadius,
        IDtQueryFilter filter, IRcRand frand, out long randomRef, out RcVec3f randomPt) => FindRandomPointAroundCircle(startRef, centerPos, maxRadius, filter, frand, DtStrictDtPolygonByCircleConstraint.Shared, out randomRef, out randomPt);
        ...
        out long randomRef, out RcVec3f randomPt)
    {
        ...
        while (!m_openList.IsEmpty())
        {
            ...
            if (bestPoly.GetPolyType() == DtPolyTypes.DT_POLYTYPE_GROUND)
            {
                // Calc area of the polygon.
                var polyArea = 0.0f;
                var polyVerts = new float[bestPoly.vertCount * 3];
                for (var j = 0; j < bestPoly.vertCount; ++j)
                {
                ...
                {
                    var vertCount = constrainedVerts.Length / 3;
                    for (var j = 2; j < vertCount; ++j)
                    {
                        var va = 0;
                        var vb = (j - 1) * 3;
                        var vc = j * 3;
            ...
            for (var i = bestTile.polyLinks[bestPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = bestTile.links[i].next)
            {
                var link = bestTile.links[i];
                var neighbourRef = link.refs;
                ...
                var total = bestNode.total + RcVec3f.Distance(bestNode.pos, neighbourNode.pos);
        ...
        var areas = new float[randomPolyVerts.Length / 3];
        var pt = DtUtils.RandomPointInConvexPoly(randomPolyVerts, randomPolyVerts.Length / 3, areas, s, t);
        _ = ClosestPointOnPoly(randomPolyRef, pt, out var closest, out _);
    ...
    public DtStatus ClosestPointOnPolyBoundary(long refs, RcVec3f pos, out RcVec3f closest)
    {
        ...
        // Collect vertices.
        var verts = new float[m_nav.GetMaxVertsPerPoly() * 3];
        var edged = new float[m_nav.GetMaxVertsPerPoly()];
        var edget = new float[m_nav.GetMaxVertsPerPoly()];
    ...
    protected void QueryPolygonsInTile(DtMeshTile tile, RcVec3f qmin, RcVec3f qmax, IDtQueryFilter filter, IDtPolyQuery query)
    {
        ...
        else
        {
            ...
            for (var i = 0; i < tile.data.Header.polyCount; ++i)
            {
                ...
                // Calc polygon bounds.
                var v = p.verts[0] * 3;
                var bmin = new RcVec3f(tile.data.Verts.AsSpan(v));
                var bmax = new RcVec3f(tile.data.Verts.AsSpan(v));
 ...
 */
    public DtStatus FindPath(long startRef, long endRef, RcVec3f startPos, RcVec3f endPos, IDtQueryFilter filter, ref List<long> path, DtFindPathOption fpo)
    {
        ...
        var raycastLimitSqr = RcMath.Sqr(raycastLimit);
        ...
        if ((options & DtFindPathOptions.DT_FINDPATH_ANY_ANGLE) != 0 && raycastLimit < 0f)
        {
            ...
            var agentRadius = tile.data.Header.walkableRadius;
        ...
        while (!m_openList.IsEmpty())
        {
            ...
            for (var i = bestTile.polyLinks[bestPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = bestTile.links[i].next)
            {
                var neighbourRef = bestTile.links[i].refs;
                ...
                if (!foundShortCut)
                {
                    var curCost = filter.GetCost(bestNode.pos, neighbourPos, parentRef, parentTile,
                ...
                {
                    // Cost
                    var endCost = filter.GetCost(neighbourPos, endPos, bestRef, bestTile, bestPoly, neighbourRef,
 ...
 * @return
 */
    public DtStatus InitSlicedFindPath(long startRef, long endRef, RcVec3f startPos, RcVec3f endPos, IDtQueryFilter filter, int options) => InitSlicedFindPath(startRef, endRef, startPos, endPos, filter, options, DtDefaultQueryHeuristic.Default, -1.0f);

    public DtStatus InitSlicedFindPath(long startRef, long endRef, RcVec3f startPos, RcVec3f endPos, IDtQueryFilter filter, int options, float raycastLimit) => InitSlicedFindPath(startRef, endRef, startPos, endPos, filter, options, DtDefaultQueryHeuristic.Default, raycastLimit);
 ...
 */
    public virtual DtStatus UpdateSlicedFindPath(int maxIter, out int doneIters)
    {
        ...
        while (iter < maxIter && !m_openList.IsEmpty())
        {
            ...
            if (parentRef != 0)
            {
                ...
                var invalidParent = status.Failed();
            ...
            for (var i = bestTile.polyLinks[bestPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = bestTile.links[i].next)
            {
                ...
                {
                    // No shortcut found.
                    var curCost = m_query.filter.GetCost(bestNode.pos, neighbourPos, parentRef, parentTile,
                        parentPoly, bestRef, bestTile, bestPoly, neighbourRef, neighbourTile, neighbourPoly);
                    cost = bestNode.cost + curCost;
                }
                ...
                {
                    var endCost = m_query.filter.GetCost(neighbourPos, m_query.endPos, bestRef, bestTile,
    ...
    public virtual DtStatus FinalizeSlicedFindPath(ref List<long> path)
    {
        ...
        else
        {
            ...
            _ = GetPathToNode(m_query.lastBestNode, ref path);
        }
        ...
        int maxStraightPath)
    {
        if (straightPath.Count > 0 && DtUtils.VEqual(straightPath[^1].pos, pos))
        {
            ...
            straightPath[^1] = new DtStraightPath(straightPath[^1].pos, flags, refs);
        }
        else
        {
        ...
        int maxStraightPath, int options)
    {
        ...
        // Add start point.
        var stat = AppendVertex(closestStartPos, DtStraightPathFlags.DT_STRAIGHTPATH_START, path[0], ref straightPath, maxStraightPath);
        if (!stat.InProgress())
        {
            return stat;
        }
                ...
                if (i + 1 < path.Count)
                {
                    ...
                    if (ppStatus.Failed())
                    {
                        ...
                        if ((options & (DtStraightPathOptions.DT_STRAIGHTPATH_AREA_CROSSINGS | DtStraightPathOptions.DT_STRAIGHTPATH_ALL_CROSSINGS)) != 0)
                        {
                            ...
                            _ = AppendPortals(apexIndex, i, closestEndPos, path, ref straightPath, maxStraightPath, options);
                        }

                        // Ignore status return value as we're just about to return anyway.
                        _ = AppendVertex(closestEndPos, 0, path[i], ref straightPath, maxStraightPath);
        ...
        // Ignore status return value as we're just about to return anyway.
        _ = AppendVertex(closestEndPos, DtStraightPathFlags.DT_STRAIGHTPATH_END, 0, ref straightPath, maxStraightPath);
        ...
        out RcVec3f resultPos, ref List<long> visited)
    {
        ...
        var tinyNodePool = new DtNodePool();

        var startNode = tinyNodePool.GetNode(startRef);
        startNode.pidx = 0;
        startNode.cost = 0;
        startNode.total = 0;
        startNode.id = startRef;
        startNode.flags = DtNodeFlags.DT_NODE_CLOSED;
        var stack = new LinkedList<DtNode>();
        _ = stack.AddLast(startNode);
        _ = new RcVec3f();
        ...
        var searchRadSqr = RcMath.Sqr((RcVec3f.Distance(startPos, endPos) / 2.0f) + 0.001f);
        ...
        while (0 < stack.Count)
        {
            ...
            for (int i = 0, j = curPoly.vertCount - 1; i < curPoly.vertCount; j = i++)
            {
                ...
                var MAX_NEIS = 8;
                var nneis = 0;
                var neis = new long[MAX_NEIS];

                if ((curPoly.neis[j] & DtNavMesh.DT_EXT_LINK) != 0)
                {
                    // Tile border.
                    for (var k = curTile.polyLinks[curPoly.index]; k != DtNavMesh.DT_NULL_LINK; k = curTile.links[k].next)
                    {
                ...
                else if (curPoly.neis[j] != 0)
                {
                    ...
                    var refs = m_nav.GetPolyRefBase(curTile) | (long)idx;
                ...
                else
                {
                    ...
                    {
                        var neighbourNode = tinyNodePool.GetNode(neis[k]);
                        ...
                        var vj = j * 3;
                        var vi = i * 3;
                        var distSqr = DtUtils.DistancePtSegSqr2D(searchPos, verts, vj, vi, out _);
        ...
        if (bestNode != null)
        {
            ...
            do
            {
                var next = tinyNodePool.GetNodeAtIdx(node.pidx);
        ...
        out RcVec3f left, out RcVec3f right)
    {
        ...
        DtLink link = null;
        for (var i = fromTile.polyLinks[fromPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = fromTile.links[i].next)
        {
        ...
        {
            for (var i = toTile.polyLinks[toPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = toTile.links[i].next)
            {
        ...
        long prevRef, out DtRaycastHit hit)
    {
        ...
        var verts = new RcVec3f[m_nav.GetMaxVertsPerPoly() + 1];
        _ = RcVec3f.Zero;
        _ = RcVec3f.Zero;
        ...
        // The API input has been checked already, skip checking internal data.
        var curRef = startRef;
        m_nav.GetTileAndPolyByRefUnsafe(curRef, out var tile, out var poly);
        ...
        while (curRef != 0)
        {
            ...
            var intersects = DtUtils.IntersectSegmentPoly2D(startPos, endPos, verts, nv, out var _, out var tmax, out var _, out var segMax);
            if (!intersects)
            {
            ...
            for (var i = tile.polyLinks[poly.index]; i != DtNavMesh.DT_NULL_LINK; i = tile.links[i].next)
            {
                ...
                if (link.side is 0 or 4)
                {
                    // Calculate link size.
                    const float s = 1.0f / 255.0f;
                    var lmin = tile.data.Verts[left + 2]
                                 + ((tile.data.Verts[right + 2] - tile.data.Verts[left + 2]) * (link.bmin * s));
                    var lmax = tile.data.Verts[left + 2]
                ...
                else if (link.side is 2 or 6)
                {
                    // Calculate link size.
                    const float s = 1.0f / 255.0f;
                    var lmin = tile.data.Verts[left]
                                 + ((tile.data.Verts[right] - tile.data.Verts[left]) * (link.bmin * s));
                    var lmax = tile.data.Verts[left]
            ...
            if ((options & DtRaycastOptions.DT_RAYCAST_USE_COSTS) != 0)
            {
                ...
                var s = RcMath.Sqr(eDir.X) > RcMath.Sqr(eDir.Z) ? diff.X / eDir.X : diff.Z / eDir.Z;
            ...
            if (nextRef == 0)
            {
                ...
                // int va = a * 3;
                // int vb = b * 3;
                var dx = verts[b].X - verts[a].X;
                var dz = verts[b].Z - verts[a].X;
        ...
        ref List<long> resultRef, ref List<long> resultParent, ref List<float> resultCost)
    {
        ...
        var radiusSqr = RcMath.Sqr(radius);
        ...
        while (!m_openList.IsEmpty())
        {
            ...
            for (var i = bestTile.polyLinks[bestPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = bestTile.links[i].next)
            {
                ...
                var cost = filter.GetCost(bestNode.pos, neighbourNode.pos, parentRef, parentTile, parentPoly, bestRef,
        ...
        ref List<long> resultRef, ref List<long> resultParent, ref List<float> resultCost)
    {
        ...
        var centerPos = RcVec3f.Zero;
        for (var i = 0; i < nverts; ++i)
        {
            centerPos += verts[i];
        }

        var scale = 1.0f / nverts;
        centerPos.X *= scale;
        centerPos.Y *= scale;
        centerPos.Z *= scale;
        ...
        while (!m_openList.IsEmpty())
        {
            ...
            for (var i = bestTile.polyLinks[bestPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = bestTile.links[i].next)
            {
                ...
                var intersects = DtUtils.IntersectSegmentPoly2D(va, vb, verts, nverts, out var tmin, out var tmax, out _, out _);
                if (!intersects)
                {
                    continue;
                }
        ...
        ref List<long> resultRef, ref List<long> resultParent)
    {
        ...
        var pa = new float[m_nav.GetMaxVertsPerPoly() * 3];
        var pb = new float[m_nav.GetMaxVertsPerPoly() * 3];
        ...
        while (0 < stack.Count)
        {
            ...
            for (var i = curTile.polyLinks[curPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = curTile.links[i].next)
            {
                ...
                var neighbourNode = tinyNodePool.GetNode(neighbourRef);
                // Skip visited.
                if ((neighbourNode.flags & DtNodeFlags.DT_NODE_CLOSED) != 0)
                {
                    continue;
                ...
                var overlap = false;
                for (var j = 0; j < resultRef.Count; ++j)
                {
        ...
        ref List<RcSegmentVert> segmentVerts, ref List<long> segmentRefs)
    {
        ...
        var ints = new List<DtSegInterval>(16);
        for (int i = 0, j = poly.vertCount - 1; i < poly.vertCount; j = i++)
        {
            ...
            else
            {
                ...
                var ivj = poly.verts[j] * 3;
                var ivi = poly.verts[i] * 3;
                var seg = new RcSegmentVert
                {
                    Vmin = new RcVec3f(tile.data.Verts.AsSpan(ivj)),
                    Vmax = new RcVec3f(tile.data.Verts.AsSpan(ivi))
                };
            ...
            for (var k = 1; k < ints.Count; ++k)
            {
                ...
                if (storePortals && ints[k].refs != 0)
                {
                    ...
                    var seg = new RcSegmentVert
                    {
                        Vmin = RcVecUtils.Lerp(tile.data.Verts, vj, vi, tmin),
                        Vmax = RcVecUtils.Lerp(tile.data.Verts, vj, vi, tmax)
                    };
                    segmentVerts.Add(seg);
                ...
                // Wall segment.
                var imin = ints[k - 1].tmax;
                var imax = ints[k].tmin;
                if (imin != imax)
                {
        ...
        out float hitDist, out RcVec3f hitPos, out RcVec3f hitNormal)
    {
        ...
        var radiusSqr = RcMath.Sqr(maxRadius);
        ...
        while (!m_openList.IsEmpty())
        {
            ...
            for (int i = 0, j = bestPoly.vertCount - 1; i < bestPoly.vertCount; j = i++)
            {
                ...
                if ((bestPoly.neis[j] & DtNavMesh.DT_EXT_LINK) != 0)
                {
                    // Tile border.
                    var solid = true;
                    for (var k = bestTile.polyLinks[bestPoly.index]; k != DtNavMesh.DT_NULL_LINK; k = bestTile.links[k].next)
                    {
                ...
                else if (bestPoly.neis[j] != 0)
                {
                    ...
                    var refs = m_nav.GetPolyRefBase(bestTile) | (long)idx;
            ...
            for (var i = bestTile.polyLinks[bestPoly.index]; i != DtNavMesh.DT_NULL_LINK; i = bestTile.links[i].next)
            {
                ...
                // Calc distance to the edge.
                var va = bestPoly.verts[link.edge] * 3;
                var vb = bestPoly.verts[(link.edge + 1) % bestPoly.vertCount] * 3;
                ...
                if (neighbourNode.flags == 0)
                {
                    _ = GetEdgeMidPoint(bestRef, bestPoly, bestTile,
                        neighbourRef, neighbourPoly, neighbourTile,
 ...
 */
    public DtStatus GetPathFromDijkstraSearch(long endRef, ref List<long> path)
    {
        ...
        var nodes = m_nodePool.FindNodes(endRef);
        if (nodes.Count != 1)
        {
            return DtStatus.DT_FAILURE | DtStatus.DT_INVALID_PARAM;
        }
        ...
        var endNode = nodes[0];
        return (endNode.flags & DtNodeFlags.DT_NODE_CLOSED) == 0
            ? DtStatus.DT_FAILURE | DtStatus.DT_INVALID_PARAM
    ...
    protected DtStatus GetPathToNode(DtNode endNode, ref List<long> path)
    {
        ...
        {
            path.Add(curNode.id);
            var nextNode = m_nodePool.GetNodeAtIdx(curNode.pidx);
            if (curNode.shortcut != null)
            {
                // remove potential duplicates from shortcut path
                for (var i = curNode.shortcut.Count - 1; i >= 0; i--)
                {
                    var id = curNode.shortcut[i];
 ...
 */
    public bool IsInClosedList(long refs)
    {
        ...
        foreach (var n in m_nodePool.FindNodes(refs))
        {
