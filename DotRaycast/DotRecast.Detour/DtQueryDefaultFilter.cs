...
public class DtQueryDefaultFilter : IDtQueryFilter
{
    ...
    public DtQueryDefaultFilter()
    {
        ...
        m_excludeFlags = 0;
        for (var i = 0; i < DtNavMesh.DT_MAX_AREAS; ++i)
        {
            m_areaCost[i] = 1.0f;
        }
    }
    ...
    public DtQueryDefaultFilter(int includeFlags, int excludeFlags, float[] areaCost)
    {
        ...
        for (var i = 0; i < Math.Min(DtNavMesh.DT_MAX_AREAS, areaCost.Length); ++i)
        {
            m_areaCost[i] = areaCost[i];
        }

        for (var i = areaCost.Length; i < DtNavMesh.DT_MAX_AREAS; ++i)
        {
            m_areaCost[i] = 1.0f;
        }
    }

    public bool PassFilter(long refs, DtMeshTile tile, DtPoly poly) => (poly.flags & m_includeFlags) != 0 && (poly.flags & m_excludeFlags) == 0;

    public float GetCost(RcVec3f pa, RcVec3f pb, long prevRef, DtMeshTile prevTile, DtPoly prevPoly, long curRef,
        DtMeshTile curTile, DtPoly curPoly, long nextRef, DtMeshTile nextTile, DtPoly nextPoly) => RcVec3f.Distance(pa, pb) * m_areaCost[curPoly.GetArea()];

    public int GetIncludeFlags() => m_includeFlags;

    public void SetIncludeFlags(int flags) => m_includeFlags = flags;

    public int GetExcludeFlags() => m_excludeFlags;
