...
public class DtNodePool
{
    ...
    public DtNode FindNode(long id)
    {
        ...
        ;
        return nodes != null && 0 != nodes.Count ? nodes[0] : null;
    }
    ...
    {
        var node = new DtNode(m_nodes.Count + 1)
        {
            id = id,
            state = state
        };
    ...
    public int GetNodeIdx(DtNode node) => node != null ? node.index : 0;

    public DtNode GetNodeAtIdx(int idx) => idx != 0 ? m_nodes[idx - 1] : null;

    public DtNode GetNode(long refs) => GetNode(refs, 0);

    public Dictionary<long, List<DtNode>> GetNodeMap() => m_map;
}
