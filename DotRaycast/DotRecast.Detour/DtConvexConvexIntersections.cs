...
public static class DtConvexConvexIntersections
{
    ...
    public static float[] Intersect(float[] p, float[] q)
    {
        var n = p.Length / 3;
        var m = q.Length / 3;
        var inters = new float[Math.Max(m, n) * 3 * 3];
        var ii = 0;
        ...
        var f = DtConvexConvexInFlag.Unknown;
        var firstPoint = true;
        var ip = new RcVec3f();
        var iq = new RcVec3f();
        ...
        do
        {
            var a = new RcVec3f(p.AsSpan(3 * (ai % n)));
            var b = new RcVec3f(q.AsSpan(3 * (bi % m)));
            var a1 = new RcVec3f(p.AsSpan(3 * ((ai + n - 1) % n)));
            var b1 = new RcVec3f(q.AsSpan(3 * ((bi + m - 1) % m)));

            var A = RcVec3f.Subtract(a, a1);
            var B = RcVec3f.Subtract(b, b1);

            var cross = (B.X * A.Z) - (A.X * B.Z); // TriArea2D({0, 0}, A, B);
            var aHB = DtUtils.TriArea2D(b1, b, a);
            var bHA = DtUtils.TriArea2D(a1, a, b);
            if (MathF.Abs(cross) < EPSILON)
            {
                cross = 0f;
            }

            var parallel = cross == 0f;
            var code = parallel ? ParallelInt(a1, a, b1, b, ref ip, ref iq) : SegSegInt(a1, a, b1, b, ref ip, ref iq);
