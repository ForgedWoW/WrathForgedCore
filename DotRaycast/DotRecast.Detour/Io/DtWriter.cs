...
public abstract class DtWriter
{
    ...
    protected void Write(BinaryWriter stream, bool @bool) => Write(stream, (byte)(@bool ? 1 : 0));

    protected void Write(BinaryWriter stream, byte value) => stream.Write(value);

    protected void Write(BinaryWriter stream, MemoryStream data)
    {
        data.Position = 0;
        var buffer = new byte[data.Length];
        _ = data.Read(buffer, 0, buffer.Length);
        stream.Write(buffer);
    }
}