...
public static class DtNavMeshBuilder
{
    ...
    public static int Subdivide(BVItem[] items, int nitems, int imin, int imax, int curNode, DtBVNode[] nodes)
    {
        ...
        {
            // Split
            var minmax = CalcExtends(items, nitems, imin, imax);
            node.bmin = minmax[0];
            node.bmax = minmax[1];

            var axis = LongestAxis(node.bmax[0] - node.bmin[0], node.bmax[1] - node.bmin[1],
    ...
    {
        // Build tree
        var quantFactor = 1 / option.cs;
        var items = new BVItem[option.polyCount];
        for (var i = 0; i < option.polyCount; i++)
        {
            var it = new BVItem();
            items[i] = it;
            it.i = i;
            ...
            {
                var vb = option.detailMeshes[(i * 4) + 0];
                var ndv = option.detailMeshes[(i * 4) + 1];
                var dv = vb * 3;
    ...
    private const int XP = 1 << 0;
    private const int ZP = 1 << 1;
    private const int XM = 1 << 2;
    private const int ZM = 1 << 3;
 ...
 */
    public static DtMeshData CreateNavMeshData(DtNavMeshCreateParams option)
    {
        ...
        if (option.offMeshConCount > 0)
        {
            ...
            {
                for (var i = 0; i < option.detailVertsCount; ++i)
                {
                    var h = option.detailVerts[(i * 3) + 1];
            ...
            {
                for (var i = 0; i < option.vertCount; ++i)
                {
                    var iv = i * 3;
                    var h = option.bmin.Y + (option.verts[iv + 1] * option.ch);
                    hmin = Math.Min(hmin, h);
                    hmax = Math.Max(hmax, h);
                }
            }
            ...
            _ = new RcVec3f();
            _ = new RcVec3f();
            var bmin = option.bmin;
            var bmax = option.bmax;
            bmin.Y = hmin;
            bmax.Y = hmax;

            for (var i = 0; i < option.offMeshConCount; ++i)
            {
        ...
        var totPolyCount = option.polyCount + storedOffMeshConCount;
        var totVertCount = option.vertCount + (storedOffMeshConCount * 2);
        ...
        for (var i = 0; i < option.polyCount; ++i)
        {
            ...
            for (var j = 0; j < nvp; ++j)
            {
                ...
                {
                    var dir = option.polys[p + nvp + j] & 0xf;
                    if (dir != 0xf)
                        portalCount++;
                }
            }
        ...
        var bvTreeSize = option.buildBvTree ? option.polyCount * 2 : 0;
        ...
        var navDVerts = new float[3 * uniqueDetailVertCount];
        var navDTris = new int[4 * detailTriCount];
        ...
        var offMeshVertsBase = option.vertCount;
        var offMeshPolyBase = option.polyCount;
        ...
        for (var i = 0; i < option.polyCount; ++i)
        {
            ...
            for (var j = 0; j < nvp; ++j)
            {
                ...
                if ((option.polys[src + nvp + j] & 0x8000) != 0)
                {
                    ...
                    var dir = option.polys[src + nvp + j] & 0xf;
                    if (dir == 0xf) // Border
                        p.neis[j] = 0;
        ...
        for (var i = 0; i < option.offMeshConCount; ++i)
        {
            ...
            if (offMeshConClass[(i * 2) + 0] == 0xff)
            {
                var p = new DtPoly(offMeshPolyBase + n, nvp);
        ...
        for (var i = 0; i < option.offMeshConCount; ++i)
        {
            ...
            if (offMeshConClass[(i * 2) + 0] == 0xff)
            {
                var con = new DtOffMeshConnection();
                offMeshCons[n] = con;
