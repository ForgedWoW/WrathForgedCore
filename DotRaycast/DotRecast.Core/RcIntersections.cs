...
public static class RcIntersections
{
    public static bool IntersectSegmentTriangle(RcVec3f sp, RcVec3f sq, RcVec3f a, RcVec3f b, RcVec3f c, out float t)
    {
        t = 0;
        float v, w;
        var ab = RcVec3f.Subtract(b, a);
        var ac = RcVec3f.Subtract(c, a);
        var qp = RcVec3f.Subtract(sp, sq);

        // Compute triangle normal. Can be precalculated or cached if
        // intersecting multiple segments against the same triangle
        var norm = RcVec3f.Cross(ab, ac);

        // Compute denominator d. If d <= 0, segment is parallel to or points
        // away from triangle, so exit early
        var d = RcVec3f.Dot(qp, norm);
        if (d <= 0.0f)
        {
            return false;
        }
    ...
    public static bool IsectSegAABB(RcVec3f sp, RcVec3f sq, RcVec3f amin, RcVec3f amax, out float tmin, out float tmax)
    {
        ...
        for (var i = 0; i < 3; i++)
        {
            ...
            {
                var ood = 1.0f / d.Get(i);
                var t1 = (amin.Get(i) - sp.Get(i)) * ood;
                var t2 = (amax.Get(i) - sp.Get(i)) * ood;
