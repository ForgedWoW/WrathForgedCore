...
namespace WrathForged.Common.Serialization.Serializers
{
    ...
    {
        public HashSet<Type> SupportedTypes { get; } = new HashSet<Type>() { typeof(ushort) };
        public HashSet<ForgedTypeCode> SupportedForgedTypeCodes { get; } = new HashSet<ForgedTypeCode>();

        public object? Deserialize(PacketBuffer packetBuffer, PropertyMeta propertyMeta, Dictionary<uint, uint> collectionSizes) => packetBuffer.Reader.ReadUInt16();
